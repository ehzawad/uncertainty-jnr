...
class JerseyCrop(A.ImageOnlyTransform):
    """Adaptive crop to focus on the jersey/torso area.

    For tall images (aspect ratio > 1.5): aggressive crop removing top 1/6 and bottom 1/3.
    For wider/shorter images: gentler crop removing top 1/10 and bottom 1/5.
    This handles different sports (soccer=tall, hockey=wide) gracefully.
    """
    ...
    def apply(self, img: np.ndarray, **params) -> np.ndarray:
        ...
        if aspect_ratio > 1.5:
            # Tall image (full-body soccer/basketball crop)
            top_crop = height // 6
            bottom_crop = height - (height // 3)
        else:
            # Wider/shorter image (hockey-style or already tight crop)
            top_crop = height // 10
            bottom_crop = height - (height // 5)

        # Safety: ensure we keep at least half the image
        if bottom_crop - top_crop < height // 2:
            top_crop = 0
            bottom_crop = height
...
def get_train_transforms(
    ...
    # Add augmentations for cross-domain robustness
    transforms.extend(
        [
            DominantColorShift(config=ColorShiftConfig(), p=p),
            A.Rotate(limit=25, p=p),
            A.Perspective(scale=(0.02, 0.06), p=p * 0.6),
            A.OneOf(
                [
