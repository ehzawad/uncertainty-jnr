...
from uncertainty_jnr.data import JerseyNumberDataset, FolderJerseyDataset, TrackletDataset, DynamicBatchSampler, tracklet_collate_fn
...
def create_datasets(
    ...
    Returns:
        If folder + max_frames > 1: (single_frame_dataset, tracklet_dataset, val_datasets_dict)
        Otherwise: (train_dataset, val_datasets_dict)
    """
    from uncertainty_jnr.augmentation import get_train_transforms, get_val_transforms

    dataset_type = getattr(config.data, "dataset_type", "soccernet")

    if dataset_type == "folder":
        return _create_folder_datasets(config, get_train_transforms, get_val_transforms)

    return _create_soccernet_datasets(
        config, val_dataset_names, train_dataset_names, masked_numbers,
        get_train_transforms, get_val_transforms,
    )


def _create_folder_datasets(config, get_train_transforms, get_val_transforms):
    """Create datasets from ImageFolder-style layout.

    When max_frames > 1, training uses TrackletDataset (5D chunks).
    Otherwise, uses FolderJerseyDataset (single-frame).
    Validation always uses FolderJerseyDataset.
    """
    folder_dir = config.data.folder_dataset_dir
    ...
    if max_frames > 1:
        ...
        train_dataset = FolderJerseyDataset(
            root_dir=folder_dir,
            split="train",
            target_size=config.data.target_size,
            transform=train_transform,
    ...
    val_datasets = {}
    for val_name in config.data.val_dataset_names:
        split = val_name if val_name in ("val", "test") else "val"
        val_datasets[val_name] = FolderJerseyDataset(
            root_dir=folder_dir,
            split=split,
            target_size=config.data.target_size,
...
def _create_soccernet_datasets(
    ...
    """Create datasets from SoccerNet CSV-based format."""
