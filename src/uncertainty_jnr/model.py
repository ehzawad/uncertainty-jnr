...
class STNJerseyModel(nn.Module):
    ...
    def __init__(
        ...
        # PARseq digit confidence scale for absent detection
        self.digit_conf_scale = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(
        ...
        # Step 6: Absent detection — combine learned head + PARseq digit confidence
        learned_absent = self.absent_head(patch_tokens)  # (B, 1)
        # PARseq digit confidence: how confident is it that position 0 has ANY digit?
        log_probs = F.log_softmax(parseq_logits, dim=-1)
        digit_conf = log_probs[:, 0, 1:11].logsumexp(dim=-1)  # (B,) in [-inf, 0]
        # Low digit_conf → high absent (negate: -digit_conf is large when no digits)
        absent_logit = learned_absent + self.digit_conf_scale * (-digit_conf.unsqueeze(1))
