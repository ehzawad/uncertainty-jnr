...
def digit_level_voting(alphas, uncertainties):
    ...
    for i in range(n):
        ...
        if pred < 10:
            # Single digit — lower confidence weight (PARseq less certain on single chars)
            tens_votes[pred] += w * 5
            single_vs_double[True] += w * 5
        else:
            # Two digit — higher confidence weight (two characters = more signal)
            tens_d = pred // 10
            ones_d = pred % 10
            tens_votes[tens_d] += w * 10
            ones_votes[ones_d] += w * 10
            single_vs_double[False] += w * 10
