...
def train_and_evaluate(
                ...
                if isinstance(vm, dict):
                    for k, v in vm.items():
                        if isinstance(v, dict) and "accuracy" in v:
                            best_val_acc = max(best_val_acc, v["accuracy"])
                if best_val_acc > 0 and rank == 0 and logger:
                    logger.info(f"Restored best val accuracy: {best_val_acc:.2f}%")
            del ckpt
    ...
    # SWA: average weights over last 25% of training for smoother generalization
    swa_model = AveragedModel(model)
    swa_start_epoch = int(config.training.max_epochs * 0.75)
    swa_active = False
    if rank == 0 and logger:
        logger.info(f"SWA will activate at epoch {swa_start_epoch + 1}")
    ...
    for epoch in range(start_epoch, config.training.max_epochs):
        ...
        # Activate SWA at 75% of training
        if epoch >= swa_start_epoch and not swa_active:
            swa_active = True
            if rank == 0 and logger:
                logger.info("SWA activated — averaging weights from now on")

        model.train()

        # Set epoch for DistributedSampler (not used with DynamicBatchSampler)
        ...
        # Calculate steps to skip for resumed mid-epoch
        steps_per_epoch = len(train_loader)
        skip_steps = global_step - epoch * steps_per_epoch if epoch == start_epoch and global_step > 0 else 0
        ...
        for step, batch in enumerate(train_loader):
            ...
            if (
                ...
                samples_per_sec = (
                    config.training.logging_steps * batch_size * world
                ) / step_time
                logger.info(
            ...
            if (
                ...
                if primary_acc > best_val_acc + min_delta:
                ...
                elif primary_acc > best_val_acc:
                    # Improved but less than min_delta
                    best_val_acc = primary_acc
                    save_checkpoint(
                        model=get_base_model(model),
                        optimizer=optimizer,
                ...
                if patience > 0 and patience_counter >= patience:
                    logger.info(
                        f"Early stopping: no improvement > {min_delta}% for "
                        f"{patience} validations. Best: {best_val_acc:.2f}%"
                    )
                    early_stop = True
                    break
        ...
        # SWA: update averaged model
        if swa_active:
            swa_model.update_parameters(model)
            if rank == 0 and logger:
                logger.info("SWA: weights averaged")
...
def train_worker(
    ...
    # Setup DDP only when actually distributed
    ...
    dataset_result = create_datasets(
        config=config,
    ...
    train_dataset, val_datasets_dict = dataset_result[0], dataset_result[1]
    ...
    max_frames = getattr(config.data, "max_frames", 1)
    use_tracklets = max_frames > 1 and hasattr(train_dataset, "samples") and hasattr(train_dataset.samples[0], "__getitem__") and "length" in train_dataset.samples[0]

    if use_tracklets:
        # DynamicBatchSampler for tracklet training
        if rank == 0 and logger:
            logger.info(f"Using DynamicBatchSampler (max_frames={max_frames})")
        batch_sampler = DynamicBatchSampler(
            train_dataset,
            max_batch_size=config.data.max_batch_size,
            min_batch_size=config.data.min_batch_size,
    ...
    # Cosine annealing with linear warmup — better for head-only training
    def lr_lambda(step):
    ...
    try:
        ...
        # Save SWA model if it was active
        if swa_active and rank == 0:
            torch.optim.swa_utils.update_bn(train_loader, swa_model, device=device)
            swa_path = run_dir / "swa_checkpoint.pt"
            torch.save({"model_state_dict": swa_model.module.state_dict()}, swa_path)
            if logger:
                logger.info(f"SWA checkpoint saved to {swa_path}")
