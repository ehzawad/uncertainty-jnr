...
class DataConfig(BaseModel):
    ...
    # Dataset selection
    dataset_type: Literal["soccernet", "folder"] = "soccernet"
    folder_dataset_dir: Optional[Path] = None  # Path for folder-type datasets
    ...
    # Tracklet training settings
    max_frames: int = 1  # 1 = single-frame mode (no tracklets)
...
class Config(BaseModel):
    ...
    def resolve_paths_in_config(cls, values: dict[str, Any]) -> dict[str, Any]:
        ...
        if data_cfg:
            ...
            data_cfg.folder_dataset_dir = _resolve_path(
                data_cfg.folder_dataset_dir, project_root
            )
        ...
        if model_cfg:
            ...
            model_cfg.parseq_weights_path = _resolve_path(
                model_cfg.parseq_weights_path, project_root
            )
